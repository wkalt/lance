...
use lance_table::format::overlay::{DataOverlayFile, OverlayCoverage, tombstone_overlay_fields};
...
/// Read each staged file's footer schema.
async fn read_staged_schemas(
    ...
    let mut schemas = Vec::with_capacity(replacements.len());
    for DataReplacementGroup(_, file) in replacements {
