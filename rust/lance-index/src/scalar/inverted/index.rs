...
impl PostingListReader {
    ...
    async fn read_chunk_batch(
        ...
        let row_range = match &self.offsets {
            // Legacy v1: the offsets table maps tokens to posting rows.
            Some(offsets) => {
            ...
            // V2: one posting row per token, so the token range is the row range.
    ...
    async fn prewarm_posting_lists_instrumented(
        ...
        // beta.4 eagerly loads offsets/max_scores/lengths in `try_new`, so
        // there is no lazy metadata to materialize here (the main-branch
        // `ensure_metadata_loaded` exists only to fill a v2 OnceCell). Clone the
        // already-resident metadata for the blocking chunk builders.
