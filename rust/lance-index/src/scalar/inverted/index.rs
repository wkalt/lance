...
impl InvertedIndex {
    ...
    pub async fn bm25_base_scorer(&self, query_tokens: &Tokens) -> Result<MemBM25Scorer> {
        let (total_tokens, num_docs) = self.aggregate_corpus_stats().await?;
        let mut token_docs = HashMap::with_capacity(query_tokens.len());
        for token in query_tokens {
            let df = self.df_for_term(token).await?;
            token_docs.insert(token.to_string(), df);
        }
